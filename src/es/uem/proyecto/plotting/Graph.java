...
/**
 * <p>
 * Genera un gráfico utilizando la API 2D de Java.
 * </p>
 * <p>
 * La personalización del gráfico se define en <code>PlotSettings</code>.
 * </p>
 * <p>
 ...
 * </p>
 *
 * @author Olly Oechsle, University of Essex (v1.1)
 * @author JM.López, Universidad Europea de Madrid (<code>drawPoint()</code>).
 * @version 1.2
 */
...
public class Graph {
    ...
    public Graph(PlotSettings p) {
        this.functions = new ArrayList<Plotter>(5);
        this.plotSettings = p;
    }
    ...
    public void draw(Graphics g, int width, int height) {
        ...
        */
        ...
        for (double px = firstGridXLocation; px <= plotSettings.getMaxX(); px += plotSettings.getGridSpacingX()) {
            ...
            value = plotSettings.numberFormatterX.format(px);
        ...
        for (double py = firstGridYLocation; py <= plotSettings.getMaxY(); py += plotSettings.getGridSpacingY()) {
            ...
            // work out the value at this point and draw
            String value = plotSettings.numberFormatterY.format(py);
    ...
    /**
     * Plots a point in the shape of a small cross '+'.
     * @author JM.López
     * 
     * @param g Graphics context upon which to write
    ...
    public void drawPoint(Graphics g, double x, double y) {
    	g.drawLine(getPlotX(x)-2, getPlotY(y), getPlotX(x)+2, getPlotY(y));
    	g.drawLine(getPlotX(x), getPlotY(y)-2, getPlotX(x), getPlotY(y)+2);
    }
